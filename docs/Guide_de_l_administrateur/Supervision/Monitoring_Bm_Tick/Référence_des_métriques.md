...
| bm-hps | service d'authentification |
| authCount | Counter | nombre de demandes de connexions à BlueMind | * status success : connexion réussie
...
| requestsCount | Counter | nombre de requêtes servies par hps | * kind : maintenance - affichage de la page de maintenance
...
| bm-lmtpd | service de remise des emails |
...
| bm-locator | localisation des services |
| executionTime | Timer | temps d'exécution des requêtes |  |
| requestsCount | Counter | nombre de requêtes reçues par le service | * origin : composant ayant émis la requête
...
| bm-xmpp | service de messagerie instantanée |
...
| authCount | Counter | nombre de demandes traitées | * status ok : demandes validées (par exemple authentification acceptée pour le login/pass entré par une personne)
...
| elasticsearch* |  | données concernant ElasticSearch | pour plus d'informations et de détails sur les métriques ES, voir la documentation dédiée &lt;https://github.com/influxdata/telegraf/tree/master/plugins/inputs/elasticsearch> |
| imapd.process |  |  |  |
