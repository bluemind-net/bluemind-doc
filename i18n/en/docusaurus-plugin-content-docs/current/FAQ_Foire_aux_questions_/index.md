...
![](../attachments/79861446/79861450.png)
...
![](../attachments/79861446/79861449.png)
...
![](../attachments/79861446/79861458.gif)
...
1 select the message in the list and click the menu "More actions" above the message list "Edit as new"
...
![](../attachments/79861446/79861466.png)
...
1 the domain administrator can authorize user-specific synchronizations in the admin console > Directory browser > select the user > "Devices" tab
...
To access shared folders on a mobile device, you must [subscribe](/Guide_de_l_utilisateur/La_messagerie/Préférences_de_messagerie/#MailPreferences-abonnements) to folders first. On Android devices, however, they may sometimes not be shown immediately. 

To display them, you have to force synchronize them first:

1 Go to the **device's** settings (not the mail application settings)
1 Go to the account section.Note: the name of this section may vary from one device to the next – "Accounts", "Accounts and Users", etc.
1 Choose the account for BlueMind syncing
1 Go to sync settings
1 Tap "*Sync now*"Note: Depending on the device, this action may be accessed directly or through a hamburger menu (three dots or bars)
...
1 If you no longer access to the administration console (after BlueMind upgrade, for example), use command line in console mode:
  - 
...
1 the activity file `/var/log/bm-mapi/activities.log`
...
![](../attachments/79861446/79861448.png)
